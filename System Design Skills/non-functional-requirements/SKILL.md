...
description: Produces a versioned Non-Functional Requirements Document (with a machine-readable data contract covering performance, scalability, availability, security, and other quality attributes) from a Functional Requirements Document, as stage 2 of a six-stage design pipeline (mini-prd → functional-requirements → non-functional-requirements → architecture-design → mermaid-js → project-readme). Use this when the user wants to define quality attributes, constraints, SLAs, throughput/latency targets, or non-functional requirements for a product that already has (or should have) a functional requirements document, or explicitly asks to run/update the "non-functional requirements" / "NFR" stage. Also use it when re-invoked by the design-pipeline-orchestrator skill. Do not use this to make architecture or technology decisions (e.g. "use S3", "use Postgres") — those belong to architecture-design.
---
