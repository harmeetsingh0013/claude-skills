...
- **The appendix covers every major technology in section 5, with no exceptions.** It's easy to write a full appendix entry for the one or two decisions that felt hard (say, the database) and shortchange the ones that felt obvious (say, the language or the CI/CD tool). Resist that — "obvious" is often exactly where a reader most wants to know *why*, especially if they'd have reached for something else. A thin appendix entry ("Python: because the team knows it. Confidence: High.") is a violation of the same standard as no entry at all — every field (problem, requirements, alternatives, evidence, trade-offs, why selected, why not the alternatives, risks, mitigation, reversibility, confidence) gets a real answer, even if some answers are short because the decision genuinely was straightforward.
- If the user challenged and you revised a decision earlier in the conversation (see `disagreement-protocol.md`), the appendix entry reflects the final reasoning after that revision — including, briefly, what the original take was and what changed it. That's more useful to a future reader than pretending the first version never happened.
